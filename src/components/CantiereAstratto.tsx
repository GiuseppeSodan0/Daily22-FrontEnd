...
export default function CantiereAstratto() {
  ...
  const zones: ZoneDetail[] = [
    {
      ...
      icon: <Layers className="w-4 h-4 text-yellow-600" />,
      riskLevel: 'Sicuro',
      color: 'yellow',
      coordinateX: 28,
      coordinateY: 25,
  ...
  return (
    <section className="py-24 bg-gray-50 relative overflow-hidden" id="cantiere-digitale-astrattismo">
      {/* Background Soft Gold Ambient Orbs */}
      <div className="absolute top-1/4 left-1/3 w-[350px] h-[350px] bg-yellow-400/[0.03] rounded-full blur-[130px] animate-pulse pointer-events-none" style={{ animationDuration: '9s' }} />
      <div className="absolute bottom-1/4 right-1/4 w-[400px] h-[400px] bg-yellow-400/[0.02] rounded-full blur-[140px] animate-pulse pointer-events-none" style={{ animationDuration: '13s' }} />

      {/* Elegant structural Grid lines */}
      <div className="absolute inset-0 bg-[linear-gradient(to_right,#00000003_1px,transparent_1px),linear-gradient(to_bottom,#00000003_1px,transparent_1px)] bg-[size:32px_32px] opacity-75 pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          ...
          <div className="lg:col-span-7 flex flex-col justify-between p-6 rounded-3xl bg-white border border-gray-200 shadow-md relative overflow-hidden group">
            
            {/* Action Bar */}
            <div className="flex items-center justify-between mb-6 pb-4 border-b border-gray-200">
              <div className="flex items-center gap-3">
                <span className="flex h-2 w-2 rounded-full bg-yellow-600 animate-ping" />
            ...
            <div className="relative flex-1 bg-gray-900 border border-gray-700 rounded-2xl p-4 flex items-center justify-center min-h-[380px] md:min-h-[440px] overflow-hidden">
              
              {/* Radar Rotating Line */}
              {isLiveSignal && (
                <div 
                  className="absolute aspect-square w-[540px] pointer-events-none opacity-15 border border-yellow-300/20 rounded-full"
                  style={{
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={`absolute -inset-4 rounded-full border ${
                        isActive ? 'scale-125 opacity-100 animate-pulse border-yellow-600/30 bg-yellow-600/[0.05]' : 'scale-50 opacity-0 group-hover/btn:scale-100 group-hover/btn:opacity-40 border-white/[0.08]'
                      } transition-all duration-300`} />

                      {/* Diagnostic Ping */}
                      {isLiveSignal && (zone.riskLevel === 'Sospeso' || zone.riskLevel === 'Attenzione') && (
                        <span className="absolute -inset-1 rounded-full bg-yellow-600 opacity-30 animate-ping pointer-events-none" />
                      )}

                      {/* Content Node Pill */}
                      <div className={`relative px-4 py-2 rounded-xl border flex items-center gap-2.5 transition-all duration-300 shadow-xl ${
                        isActive
                          ? `bg-gray-900 border-yellow-500 text-yellow-400 scale-110 z-10 font-bold`
                          : `bg-gray-900/90 border-white/[0.08] text-gray-300 hover:text-white hover:border-white/[0.2]`
                      }`}>
                        {zone.icon}
              ...
              <div className="absolute right-4 bottom-4 flex flex-col items-center gap-1 opacity-20 select-none">
                ...
                <span className="text-[7px] font-mono text-gray-400">ST-GRID</span>
              </div>
            </div>
            ...
            <div className="mt-6 p-4 rounded-xl bg-gray-50 border border-gray-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <p className="text-[10px] font-medium text-gray-400 uppercase font-mono">Stato Gateway dailyplatform</p>
                  <p className="text-[11px] text-gray-600 font-medium">Wearable WIDIU attivi ed allineati via ripetitori radio di cantiere</p>
                </div>
              </div>
          ...
          <div className="lg:col-span-5 flex flex-col justify-between p-6 md:p-8 rounded-3xl bg-white border border-gray-200 shadow-md h-full relative">
              ...
              >
                <div>
                  <div className="flex items-center justify-between gap-3 mb-6">
                    <div className="flex items-center gap-2.5">
                      <span className="p-2.5 rounded-xl bg-gray-50 border border-gray-200">
                        {activeZone.icon}
                      </span>
                      <div>
                        <span className="text-[9px] font-bold font-mono text-gray-400 uppercase tracking-widest block">REPARTO MONITORTATO</span>
                        <h3 className="text-base font-bold text-gray-900 leading-tight mt-0.5">{activeZone.name}</h3>
                      </div>
                    </div>
                  ...
                  {/* Operative Actions */}
                  <div className="mb-6">
                    <p className="text-[10px] text-gray-500 uppercase font-mono font-bold tracking-widest mb-2">Mansioni Operative</p>
                  ...
                  {/* Telemetry data info */}
                  <div className="mb-6">
                    <p className="text-[10px] text-gray-500 uppercase font-mono font-bold tracking-widest mb-3">Telemetria Sensori (D&amp;S)</p>
                    <div className="grid grid-cols-2 gap-3">
                      {activeZone.parameters.map((param, index) => (
                        <div key={index} className="p-3 rounded-xl bg-gray-50 border border-gray-100 hover:bg-gray-100 transition-colors">
                          <p className="text-[9px] text-gray-400 uppercase font-mono">{param.label}</p>
                  ...
                  <div className="mb-6">
                    <div className="flex items-center gap-1.5 mb-2.5">
                      ...
                      <p className="text-[10px] text-gray-500 uppercase font-mono font-bold tracking-widest">DPI Obbligatori (D.Lgs. 81/08)</p>
                    </div>
                    <ul className="space-y-1.5">
                      {activeZone.requiredDPI.map((dpi, idx) => (
                        <li key={idx} className="flex items-start gap-2.5 text-xs text-gray-600">
                          <span className="w-1.5 h-1.5 rounded-full bg-yellow-600 mt-1.5 shrink-0" />
                          <span>{dpi}</span>
                        </li>
                      ))}
                    </ul>
                ...
                <div className="pt-6 border-t border-gray-200 bg-gradient-to-r from-yellow-50 to-transparent p-4 rounded-2xl border-l-4 border-l-yellow-400">
                  <div className="flex items-center gap-2 mb-2">
                    <Shield className="w-4 h-4 text-yellow-600" strokeWidth={2} />
                    <p className="text-[10px] text-gray-900 font-bold uppercase font-mono tracking-wider">Integrazione Predittiva Daily</p>
                  </div>
